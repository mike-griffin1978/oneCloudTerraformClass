...
# The Output variables that follow are found in the ec2_instance documentation site within the Outputs tab
# A rich number of additonal outputs are available and the usage/ease of using such outputs is a compelling justification of such
# parent module usage in resource builds

# The output names are arbitrary and could be anything that is descript of the purpose
output "ec2_instance_public_ip" {
  ...
  # value but rather is a list of values - I.e. a list and possibly multiple public IP addeesses may be returned.  The fact that this
  ...
  # - public_ip specifies the output - as exposed by the module - that we are interested in.  A full list of exposed outputs is
