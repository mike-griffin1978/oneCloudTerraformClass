...
####Example - aws_vpc resource block - replace both the resource name (vpc-dev-0) and the tags Name
####value (vpc-dev-0) to include your own pod number such as - "vpc-dev-<pod-number>"
####If preferred and for expidency/precision - considering doing a replace all such as - replace all 
####occurances of -0 with your pod number
